...
const data_price_type = ref([{ code: '1', name: 'ปรกติ' }, { code: '2', name: 'ตามกลุ่ม' }, { code: '3', name: 'ตามเจ้าหนี้' }]);
...
onMounted(() => {
  storeApp.setActivePage("purchase_list");
  storeApp.setActiveChild('');
  storeApp.setPageTitle("รายการสินค้าราคาซื้อ");
...
async function getItemPriceList() {
  ...
  await MasterdataService.getItemPricePurList(filterData.value.search, sale_type, trans_type, price_type, itembrand, itempattern, itemmodel, itemcategory, itemdesign)
    .then((res) => {
      console.log(res);
      if (res.success) {
  ...
  if (selectedProduct.value.length > 0) {
    ...
    selectedProduct.value.forEach((item) => {
      var data = {
        ...
        รหัสเจ้าหนี้: item.supplier_code.toString(),
        ส่วนลด: item.discount.toString(),
      }
      excel_data.push(data);
    });
    const worksheet = XLSX.utils.json_to_sheet(excel_data);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Purchase Price List");

    XLSX.writeFile(workbook, "ราคาซื้อ.xlsx");
  }
}
        ...
        <div class="surface-card shadow-2 border-round p-fluid mt-2">
          ...
          <Panel header="ปรับราคา และ ส่งออก" :toggleable="true" :collapsed="false">
            <div class="grid formgrid p-fluid">
              ...
              <div class="field col-6 md:col-2">
                <label class="font-medium text-900">ส่วนลด</label>
                <InputText type="text" v-model="updateData.discount" />
              </div>
              <div class="field col-6 md:col-2">
