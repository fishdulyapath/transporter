...
onMounted(() => {
  ...
  storeApp.setActiveChild("purchase_doc_list");
  storeApp.setPageTitle("รายการนำเข้าราคาซื้อ");
...
function goToImport() {
  router.push({ name: 'purchase_import' });
}
async function getDocList() {
  ...
  await MasterdataService.getDocPurList(search.value, from_datex, to_datex)
    .then((res) => {
      console.log(res);
      if (res.success) {
...
function viewDetails(docNo) {
  ...
  router.push({ name: "purchase_detail", params: { id: docNo } });
};
        ...
        <Panel header="การค้นหา" :toggleable="true">
          ...
          <div class="flex justify-content-start">
            <div class="flex-none flex">
              ...
              <Button label="นำเข้าราคาซื้อ" icon="pi pi-upload" class="w-auto p-button-info ml-2" @click="goToImport"></Button>
            </div>
          </div>
        </Panel>
