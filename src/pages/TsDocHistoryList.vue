...
const statusDetails = ref([
  { code: "", name: "ไม่เลือก" },
  { code: "0", name: "รอส่ง" },
  { code: "1", name: "รออนุมัติ" },
  { code: "2", name: "อนุมัติแล้ว" },
...
onMounted(() => {
  storeApp.setActivePage("tsdochistorylist");
  storeApp.setActiveChild('');
  storeApp.setPageTitle("ประวัติการขนส่งสินค้า");
...
const getDocList = async () => {
  ...
  await MasterdataService.getTsDocHistoryList(search.value, from_datex, to_datex, status_search.value)
    .then((res) => {
      console.log(res);
      if (res.success) {
...
const viewDetails = async (doc_no) => {
  console.log(doc_no);
  router.push({ name: "tshistorydetail", params: { id: doc_no } });
};
        ...
        <div class="surface-card shadow-2 border-round p-fluid mt-2">
          <Panel header="ค้นหาสินค้า" :toggleable="true">
            <div class="grid formgrid p-fluid">
              ...
              <div class="field mb-4 col-12 md:col-3">
                ...
                <Dropdown v-model="status_search" :options="statusDetails" optionLabel="name" optionValue="code" placeholder="เลือกสถานะ" />
              </div>
          ...
          <DataTable :value="data_list" :paginator="true" :rows="15"
            ...
            <Column field="status" header="สถานะ" sortable>
              <template #body="{ data }">
                <span v-if="data.status == '0'" class="p-tag p-tag-rounded p-tag-info">รอส่ง</span>
                <span v-else-if="data.status == '1'" class="p-tag p-tag-rounded p-tag-warning ">รออนุมัติ</span>
                <span v-else-if="data.status == '2'" class="p-tag p-tag-rounded p-tag-success">อนุมัติแล้ว</span>
                <span v-else-if="data.status == '3'" class="p-tag p-tag-rounded p-tag-danger">ไม่อนุมัติ</span>
              </template>
            </Column>
            <Column field="approve_code" header="ผู้อนุมัติ" sortable>
              <template #body="{ data }">
                {{ getEmployeeName(data.approve_code) }}
              </template>
            </Column>
            <Column field="approve_date" header="วันที่อนุมัติ" sortable>
              <template #body="{ data }">
                {{ Utils.getDateFormatPG(data.approve_date) }}
              </template>
            </Column>
            <Column field="" header="">
              <template #body="slotProps">
