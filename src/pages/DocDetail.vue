...
onMounted(async () => {
  ...
  storeApp.setActivePage("doc_list");
...
const openRemarkDialog = (dialogData) => {
  data.value = dialogData;
  remark.value = dialogData.remark;
...
const saveRemark = () => {
  data.value.remark = remark.value;

  console.log(data.value);
  var post_data = {
    doc_no: data.value.doc_no,
    roworder: data.value.roworder,
    remark: data.value.remark,
  };

  MasterdataService.updateDetailRemark(post_data)
    .then((res) => {
      console.log(res);
      if (res.success) {
        closeRemarkDialog();
  ...
  <AppLayout>
    <MainContentWarp>
      <div class="surface-ground px-2 py-2">
        <Dialog v-model:visible="remarkDialogVisible" :style="{ width: '450px' }" header="หมายเหตุ" :modal="true">
          <div class="grid">
            <div class="col-12">
              <InputText type="text" v-model="remark" class="w-full" />
            </div>
            <div class="col-12 flex justify-content-between">
              <Button label="ยกเลิก" class="p-button-danger" @click="closeRemarkDialog" />
              <Button label="บันทึก" class="p-button-success" @click="saveRemark" />
            </div>
          </div>
        </Dialog>
        ...
        <div class="surface-card shadow-2 border-round p-fluid p-2">
          ...
          <div class="mt-2 mb-2">
            <Button @click="showConfirmCreate" label="ส่งรายการนับซ้ำ" icon="pi pi-save" class="w-auto p-button-danger"></Button>
            <Button @click="exportCSV" label="Export" icon="pi pi-external-link" class="w-auto p-button-info ml-1"></Button>
          </div>
          <DataTable v-model:selection="selectedProduct" :value="doc_detail" dataKey="roworder" showGridlines ref="dt">
            <Column selectionMode="multiple" headerStyle="width: 3rem"></Column>
            <Column header="" style="width: 50px">
              <template #body="slotProps">
                <Button icon="pi pi-tag" rounded class="p-button-text p-button-sm p-button-warning" @click="openRemarkDialog(slotProps.data, slotProps.data.remark)" />
              </template>
            </Column>
