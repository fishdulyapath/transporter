...
function goToImport() {
  router.push({ name: 'sale_std_import' });
}
async function getDocList() {
  ...
  await MasterdataService.getDocStdList(search.value, from_datex, to_datex)
    .then((res) => {
      console.log(res);
      if (res.success) {
...
function viewDetails(docNo) {
  ...
  router.push({ name: "sale_std_import_detail", params: { id: docNo } });
};
