...
onMounted(() => {
  ...
  storeApp.setActiveChild("sale_std_import");
  storeApp.setPageTitle("รายการนำเข้าราคามาตรฐาน");
  storeApp.setHideToggle();
  doc_no.value = Utils.getDocNoDate('SP');
  ...
  reader.onload = function (e) {
    ...
    for (let index = 0; index < results.length; index++) {
      ...
      details.push({
        ...
        item_name: results[index].ชื่อสินค้า != undefined ? results[index].รหัสสินค้า.toString().trim() : "",
...
function goList() {
  router.push({ name: 'sale_std_import_list' });
}
...
const confirmSave = async () => {
  if (userInput.value === captcha.value) {
    ...
    await MasterdataService.createImportDocStd(post_data)
      .then((res) => {
        console.log(res);
        if (res.success) {
          confirmDialog.value = false;
            ...
            <div class="flex justify-content-start">
              <div class="flex-none flex">
                ...
                <FileUpload mode="basic" name="input file" accept=".xls,.xlsx" ref="myFiles" :customUpload="true" @change="ImportFile()"
                  class="p-button-plain p-button-primary ml-2" chooseIcon="pi pi-upload" chooseLabel="นำเข้าราคา"></FileUpload>
              </div>
            </div>
          </Panel>
