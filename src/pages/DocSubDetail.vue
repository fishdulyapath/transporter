...
function goList() {
  router.push({ name: "dochistory_list" });
}

function getDocData() {
  MasterdataService.getDocSubData(doc_no.value)
    .then((res) => {
      console.log(res);
      if (res.success) {
...
function getDocDetail() {
  MasterdataService.getDocSubDetail(doc_no.value)
    .then((res) => {
      console.log(res);
      if (res.success) {
        console.log(res.data);
        ...
        <div class="surface-card shadow-2 border-round p-fluid p-2">
          <div>
            <p>
              ...
              <span style="margin-left: 10px"><b>ผู้จัดทำ:</b> {{ doc_data.creator_code }}~{{ doc_data.creator_name }}</span>
            </p>
            <p>
